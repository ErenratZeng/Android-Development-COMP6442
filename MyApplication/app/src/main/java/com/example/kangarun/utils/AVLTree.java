...
public class AVLTree<T extends Comparable<T>> extends BinarySearchTree<T> {
    ...
    public AVLTree<T> insert(T element) {
        ...
        if (element == null) throw new IllegalArgumentException("Input cannot be null");

        AVLTree<T> result = this;

        // Insert and construct a new tree structure without mutating the current tree
        if (element.compareTo(value) < 0) {
            if (leftNode instanceof EmptyTree) {
                result = new AVLTree<>(value, new AVLTree<>(element), rightNode);
            } else {
            ...
            if (rightNode instanceof EmptyTree) {
                result = new AVLTree<>(value, leftNode, new AVLTree<>(element));
            } else {
                result = new AVLTree<>(value, leftNode, rightNode.insert(element));
            }
        }

        // Rebalance if necessary and return the new tree
    ...
    private AVLTree<T> rebalance() {
        ...
        if (balance > 1) {
            if (leftNode instanceof AVLTree && ((AVLTree<T>)leftNode).getBalanceFactor() < 0) {
                AVLTree<T> newLeft = leftNode instanceof AVLTree ? ((AVLTree<T>)leftNode).leftRotate() : (AVLTree<T>)leftNode;
                leftNode = newLeft;
                return this.rightRotate();
            }
            return this.rightRotate();
        } else if (balance < -1) {
            if (rightNode instanceof AVLTree && ((AVLTree<T>)rightNode).getBalanceFactor() > 0) {
                AVLTree<T> newRight = rightNode instanceof AVLTree ? ((AVLTree<T>)rightNode).rightRotate() : (AVLTree<T>)rightNode;
                rightNode = newRight;
    ...
    public AVLTree<T> leftRotate() {
         ...
         */
        if (!(rightNode instanceof AVLTree)) return this;
    ...
    public AVLTree<T> rightRotate() {
        /*
            ...
            than something about your code is incorrect!
         */
        if (!(leftNode instanceof AVLTree)) return this;
